...
import type { AiRunProgress, PublicAiRunEvent, PublicAiErrorCode } from '@aimani-gs/contracts';
  ...
  useEffect(() => {
    ...
    source.onopen = () => {
      ...
      setState((prev) => {
        ...
        const s = lastRunStatusRef.current;
        return { runId, value: { status: s } as AiRunProgress };
      });
    };
